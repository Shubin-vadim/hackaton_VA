...
class BrowserModel:
    def __init__(self):
        self.recogn_voice = VoiceRecognationModel()
    ...
    def weather(self, city="Ярославль"):
        ...
        if flag is None:
            ...
            complete_url = base_url + "appid=" + api_key + "&q=" + weather_city
        ...
        if data["cod"] == 404:
            ...
            answer_text = "В городе" + " " + city + " " + "температура составляет" + " " + str(temperature) \
                + ",\n влажность " + str(humidity) + ".\nОписание: " + self.translate_phrase(weather_description)

            return answer_text

    # выявление многозначности или однозначности слов пользователя
    def definition_of_plural(self, text):
        ...
        return not inflect.singular_noun(str(text_translate))
