...
class RecognationObjectsModel:
    ...
    def findObjects(self, outputs, img):  # метод для нахождения объекто
        ...
        nmsThreshold = 0.2
        classNames = []
        classFile = r'app\config\coco.names'  # файл с именами объектов распознавания
        ...
        for i in indices:
            ...
            cv2.rectangle(img, (x, y), (x + w, y + h), (255, 0, 255), 2,
                          cv2.LINE_AA)  # рисуем ограничивающий прямоугольник
            cv2.putText(img, f'{classNames[classIds[i]].upper()}{int(confs[i] * 100)}%', (x, y - 10),
                        cv2.FONT_HERSHEY_PLAIN, 1.5, (255, 0, 0),
                        2)  # вставляем наименование объекта и его вероятность принадлежности
            self.object = classNames[classIds[i]].upper()
            self.message.speak(
                "Впереди " + self.browser.translate_phrase(self.object))  # воспроизведение наименования объекта

    def recogn_objects(self):
        whT = 320
        modelConfiguration = r'app\NeuralNet\yolov3-tiny.cfg'  # наименование конфигурационного файла
        modelWeights = r'app\NeuralNet\yolov3-tiny.weights'  # наименование файла весов

        net = cv2.dnn.readNetFromDarknet(modelConfiguration,
                                         modelWeights)  # чтение нейронной сети с помощью её конфига и весов
        ...
        while True:  # покадровое чтение с веб камеры
            if webcam:
                ...
                frame = cv2.imread(r'https://bugaga.ru/uploads/posts/2017-01/1483531805_dvulikaya-koshka-yana-8.jpg')
            blob = cv2.dnn.blobFromImage(frame, 1 / 255, (whT, whT), [0, 0, 0], 1,
                                         crop=False)  # конвертирование изображения в формат данных NN
            net.setInput(blob)  # установка формата данных сети
            layersNames = net.getLayerNames()  # получение имен слоёв сети

            outputNames = [(layersNames[i - 1]) for i in
                           net.getUnconnectedOutLayers()]  # получение имен сети только выходных слоёв
